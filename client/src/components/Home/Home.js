...
const Home = () => {
  ...
  const createClass = async () => {
    ...
    await axios.post('http://localhost:3152/create_class', {
      userId, title
    });
  }
  ...
  const joinClass = async () => {
    ...
    await axios.post('http://localhost:3152/add_class', {
      userId, classId
    });
  };

  const fetchClasses = async () => {
    const res = await axios.get('http://localhost:3152/get_classes/' + userId);
